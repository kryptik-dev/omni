...
function Scene({
  ...
  useFrame((_, delta: number) => {
    ...
    curInRef.current += (targetIn - curInRef.current) * 0.15 // Smooth interpolation
    curOutRef.current += (targetOut - curOutRef.current) * 0.15

    const targetSpeed = 0.1 + (1 - Math.pow(curOutRef.current - 1, 2)) * 0.9
    animSpeedRef.current += (targetSpeed - animSpeedRef.current) * 0.12

    // REACTIVITY BOOST: Multiply animation speed by volume intensity
    const intensity = Math.max(curInRef.current, curOutRef.current);
    u.uAnimation.value += delta * animSpeedRef.current * (1 + intensity * 3.0);

    u.uInputVolume.value = curInRef.current
    u.uOutputVolume.value = curOutRef.current
    u.uColor1.value.lerp(targetColor1Ref.current, 0.2) // Fast color shift
    u.uColor2.value.lerp(targetColor2Ref.current, 0.2)
  })
...
float sharpRing(vec3 decomposed, float time) {
    float ringStart = 1.0;
    float ringWidth = 0.3;
    float noiseScale = 8.0; // Sharper noise
...
void main() {
    ...
    theta += noise * mix(0.02, 0.1, uOutputVolume + uInputVolume);

    // Initialize the base color to transparent so the black container shows through
    ...
    for (int i = 0; i < 7; i++) {
        ...
        // SMOOTH PETALS: Adjusted for wider, softer lobes
        b = noise * mix(2.0, 4.0, uInputVolume); 
